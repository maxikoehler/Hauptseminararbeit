...
\commenting{Input of basic knowledge for system modelling; Maybe supplementary knowledge}
...
\begin{figure}[H]
        ...
        \begin{circuitikz}[european, scale=.9, smallR/.style={resistor,resistors/scale=.7}]
                ...
                \bushere{1.75}{}{} coordinate(b1) ++(0,-1) -- ++(.25,0) -- coordinate(f2) ++(1.75,0)
                to[L, name=X_l3] ++(1,0) -- ++(1.5,0) ++(0,1)
                \bushere{1.75}{}{} ++(0,1) coordinate(b2) ++(0,-1) coordinate(b3) -- ++(.75,0) coordinate(f3) -- ++(.75,0) to[L, name=X_ibb] ++(1,0) -- ++(1,0)
...
\end{figure}
\mycomment[MK]{Update and correct SMIB model graphic}
% where
...
\begin{figure}[h]
        ...
        \begin{tikzpicture}[node distance = 1.5cm, auto]
                ...
                \node [papProcess, below of = start, yshift= -2mm,label={[shift={(2.7,-0.6)}]\footnotesize\textit{init()}}] (pro1){initialize start and simulation parameters};
                \node [papPredProc, below of = pro1, yshift= -5mm,label={[shift={(2.7,-0.6)}]\footnotesize\textit{odeint()}}] (pro2){\nodepart{two} \shortstack{do \acs{TDS} with\\constant fault}};
                \node [papProcess, below of = pro2, yshift= -5mm,label={[shift={(2.7,-0.6)}]\footnotesize\textit{determine\_cct()}}] (pro3){determine \acs{CCT} and critical angle};
                \node [papPredProc, below of = pro3, yshift= -7mm](pro4){\nodepart{two}\shortstack{solving \acs{TDS}\\stable and unstable}};
                \node [papProcess, below of = pro4, yshift= -5mm,label={[shift={(2.3,-0.6)}]\footnotesize\textit{label 3}}](pro5){plot results};
...
\commenting{Describing the algebraic equation system and how to implement in python
...
Other variables changing the \acs{CCT} of a generator are the power difference $\Delta P$ and $H_\mathrm{gen}$ (see \autoref{eq:swing2}). The resulting \acs{CCT} while variing this parameters in the ranges
\begin{align}
...
\begin{figure}[H]
        ...
        \caption[Influence of parameter variation on the \acs{CCT}]{Influence of variing the parameters power difference $\Delta P$ and $H_\mathrm{gen}$ on the \acf{CCT}}
